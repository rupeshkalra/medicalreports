...
const Edit=({navigation,route})=>{
  ...
  const update=async()=>{
    try {
      ...
      await AsyncStorage.setItem('@reports_list',JSON.stringify(list));
  ...
  useEffect(()=>{
    ...
    const {id,name,image,description,medicines}=report;
  ...
  return (
    <ScrollView style={{flex: 1}} contentContainerStyle={styles.container}>
      {cameraclicked == true ? (
        ...
        <>
          <H1 style={styles.heading}>Edit Report</H1>
          <Form>
            ...
            <Button rounded block style={styles.formItem} onPress={getCamera}>
              <Text style={{color: '#eee'}}>Update Photo</Text>
            </Button>
            {medicines.length == 0 ? (
            ...
            <Button rounded block style={{marginBottom: 20}} onPress={()=>{update()}}>
